...
test("payouts: To Pay units per seller with their Payout Info, the seller owed most on top", async () => {
  const { airtable } = fakeBase({
    ...
    "Inventory Units": [
      unit("recUNIT0000000001", { "Unfulfilled Orders Log": ["recO"], "Shipping Status": ["Delivered"], "Store Name": ["Genky"], "Shopify Order Number": ["7001"] }),
  ...
  assert.equal(all.groups[1].rows[0].reference, "Genky · 7001");
